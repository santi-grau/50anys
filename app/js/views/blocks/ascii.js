...
Ascii.prototype.refresh = function(  ){
	...
	for( var i = 1 ; i < totalLines ; i++ ){
		var w = (this.parent.parent.parent.simplexNoise.noise2D(0,i + Math.floor( this.time )) + 1) / 2;
		if( w < 0.1 ) w = 0;
		w *= ( this.block.w - lineHeight );
		rect = this.parent.parent.parent.two.makeRectangle( this.block.x + lineHeight / 2 + w / 2, this.block.y + lineHeight * i, w, lineHeight * 0.8 );
		rect.noStroke();
		rect.fill = '#ffffff';
		this.lines.add(rect);
	}
...
Ascii.prototype.exportPDF = function( block, doc, scale, strokeWidth, patterns ){
	...
	for( var i = 1 ; i < totalLines ; i++ ){
		var w = (this.parent.parent.parent.simplexNoise.noise2D(0,i ) + 1) / 2;
		if( w < 0.1 ) w = 0;
