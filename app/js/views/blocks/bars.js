var Bars = function( parent, block ) {
	...
	for( var i = 0 ; i < this.block.w; i+= size ){
		for( var j = 0 ; j < this.block.h ; j+= size ){
			...
			var s = this.parent.parent.two.makeRectangle( p.x, p.y, size, size );
			s.linewidth = 1;
			s.p = p;
			this.squares.add( s );
		}
	...
	this.group = this.parent.parent.two.makeGroup( this.squares, this.rect );
...
Bars.prototype.shade = function( val ){
	for( var i = 0 ; i < this.squares.children.length; i++ ){
		...
		var n = Math.round( ( this.parent.parent.simplexNoise.noise2D( sq.p.x / 100 + this.time, sq.p.y / 100 ) + 1) / 2 );
